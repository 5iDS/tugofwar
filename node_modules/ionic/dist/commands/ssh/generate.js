...
let SSHGenerateCommand = class SSHGenerateCommand extends base_1.SSHBaseCommand {
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const { getGeneratedPrivateKeyPath } = yield Promise.resolve().then(() => require('@ionic/cli-utils/lib/ssh'));
            const { bits, annotation } = options;
            const keyPath = inputs[0] ? expandPath(String(inputs[0])) : yield getGeneratedPrivateKeyPath(this.env);
            const keyPathDir = path.dirname(keyPath);
            const pubkeyPath = `${keyPath}.pub`;
            if (!(yield fs_1.pathExists(keyPathDir))) {
                yield fs_1.fsMkdirp(keyPathDir, 0o700);
                this.env.log.info(`Created ${chalk_1.default.bold(prettyPath(keyPathDir))} directory for you.\n`);
            }
            if (yield fs_1.pathExists(keyPath)) {
                const confirm = yield this.env.prompt({
                    type: 'confirm',
                    name: 'confirm',
                    message: `Key ${chalk_1.default.bold(prettyPath(keyPath))} exists. Overwrite?`,
                });
                if (confirm) {
                    yield fs_1.fsUnlink(keyPath);
                }
                else {
                    this.env.log.info(`Not overwriting ${chalk_1.default.bold(prettyPath(keyPath))}.`);
                    return;
                }
            }
            this.env.log.info(`You will be prompted to provide a ${chalk_1.default.bold('passphrase')}, which is ` +
            ...
            this.env.log.ok('A new pair of SSH keys has been generated!\n' +
                `Private Key (${chalk_1.default.bold(prettyPath(keyPath))}): Keep this safe!\n` +
                `Public Key (${chalk_1.default.bold(prettyPath(pubkeyPath))}): Give this to all your friends!`);
            this.env.log.info('Next steps:\n' +
                `- Add your public key to Ionic: ${chalk_1.default.green('ionic ssh add ' + prettyPath(pubkeyPath))}\n` +
                `- Use your private key for secure communication with Ionic: ${chalk_1.default.green('ionic ssh use ' + prettyPath(keyPath))}`);
        });
    }
};
SSHGenerateCommand = tslib_1.__decorate([
    command_1.CommandMetadata({
        ...
        options: [
            {
                name: 'type',
                description: `The type of key to generate: ${SSH_KEY_TYPES.map(v => chalk_1.default.green(v)).join(', ')}`,
                default: 'rsa',
                aliases: ['t'],
                advanced: true,
            },
